...
model, history = train_loop(
    ...
    trainloader,
    validloader,
    save_model_name="./mobilenetv2_fish_scratch.pt",
    max_epochs_stop=10,
    num_epochs=30,
    num_epochs_report=3)
...
axs[0].legend()
axs[0].set_title('Fish breed Classifier model loss')
...
axs[1].legend()
axs[1].set_title('Fish breed Classifier accuracy')
...
m1_test_class_acc, m1_test_acc = Accuracy_report(loader = testloader,model = model, n_classes = 9)
...
import requests
from io import BytesIO
url = "https://cdn.britannica.com/43/9443-050-5D22A052/Black-sea-bass.jpg"
...
torch.save(model,"mobilenetv2_fish_scratch.pb")

fetch_model=torch.load('../input/mobilenetv2/mobilenetv2_fish_scratch.pb')
